    ...
    def stream(self):
        ...
        if self.settings.get_webcam() and self.settings.get_watermark():
            command = str(
                ...
                + '-framerate " %(fps)s" -i :0.0+%(x_offset)s,%(y_offset)s -f '
                + 'video4linux2 -video_size %(web_resolution)s -framerate '
                + '%(fps)s -i /dev/video0 -i %(watermark_file)s -f pulse -ac 1'
                + '-i default -vcodec %(video_container)s -filter_complex '
                + "'setpts=PTS-STARTPTS[bg]; setpts=PTS-STARTPTS[fg]; "
                ...
                + '-b:a %(audio_bitrate)s -b:v %(bitrate)s '
                + '-minrate %(bitrate)s -g %(keyint)s -pix_fmt yuv420p '
                + '-f %(video_container)s "%(service)s' + twitch_key + '"'
            ) % parameters
        elif self.settings.get_watermark():
            command = str(
                'avconv -f x11grab -show_region %(show_region)s -s %(inres)s '
                + '-framerate " %(fps)s" -i :0.0+%(x_offset)s,%(y_offset)s -i '
                + '%(watermark_file)s -f pulse -ac 1 -i default -vcodec '
                + '%(video_codec)s -filter_complex '
                + "'overlay=0:main_h-overlay_h-0'"
                + ' -s %(outres)s -preset %(quality)s -acodec %(audio_codec)s '
                + '-ar 44100 -threads %(threads)s -qscale 3 '
                + '-b:a %(audio_bitrate)s -b:v %(bitrate)s -minrate '
                + '%(bitrate)s -g %(keyint)s -pix_fmt yuv420p -f '
                + '%(video_container)s "%(service)s' + twitch_key + '"'
            ) % parameters
            ...
            command = str(
                ...
                + '-framerate " %(fps)s" -i :0.0+%(x_offset)s,%(y_offset)s -f '
                + 'video4linux2 -video_size %(web_resolution)s -framerate '
                + '%(fps)s -i /dev/video0 -f pulse -ac 1 -i default -vcodec '
                + '%(video_codec)s -filter_complex '
                + "'overlay=%(web_placement)s,format=yuv420p[out]' -map "
                + "'[out]'"
                + ' -s %(outres)s -preset %(quality)s -acodec %(audio_codec)s '
                + '-ar 44100 -threads %(threads)s -qscale 3 '
                + '-b:a %(audio_bitrate)s -b:v %(bitrate)s -minrate '
            ...
            command = str(
                'avconv -f x11grab -show_region %(show_region)s -s %(inres)s '
                + '-framerate " %(fps)s" -i :0.0+%(x_offset)s,%(y_offset)s -f '
                + 'pulse -ac 1 -i default -vcodec %(video_codec)s -s '
                + '%(outres)s -preset %(quality)s -acodec %(audio_codec)s -ar '
                + '44100 -threads %(threads)s -qscale 3 -b:a '
                + '%(audio_bitrate)s -b:v %(bitrate)s -minrate %(bitrate)s -g '
                + '%(keyint)s -pix_fmt yuv420p -f %(video_container)s '
                + '"%(service)s' + twitch_key + '"'
            ) % parameters
        print (command)
